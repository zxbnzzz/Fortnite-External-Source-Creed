...
bool ImGui::Keybind(const char* label, int* key, int* mode, bool show_label)
{
    ...
    ImRect clickable(ImVec2(rect.Max.x - 30 - label_size.x, rect.Min.y), { rect.Max.x - 10, rect.Max.y });
    ...
    window->DrawList->AddRect(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y), clickable.Max - ImVec2(0, 0), GetColorU32(c::keybind::background_outline), c::keybind::rounding);
    ...
    RenderTextClipped(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y), clickable.Max, buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
    ...
    PushStyleColor(ImGuiCol_WindowBg, c::combo::background_outline);
    if (it_anim->second.alpha >= 0.01f)
    {
        SetNextWindowPos(ImVec2(clickable.GetCenter().x - 43, clickable.Max.y + 5));
...
struct button_state
{
    ImVec4 background, text, text_opacity;
};
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent_color_low : c::button::background_outline, g.IO.DeltaTime * 6.f);
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, IsItemActive() ? ImColor(205, 205, 205, 255) : ImColor(128, 128, 128, 200), g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(it_anim->second.background), c::button::rounding);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");

    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text_opacity), label);
...
bool ImGui::Tab(bool selectable, const char* label, const ImVec2& size_arg)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = ImGui::CalcTextSize(label, NULL, true), pos = window->DC.CursorPos;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selectable ? c::accent_color_low : c::tab::background_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.line = ImLerp(it_anim->second.line, selectable ? c::accent_color : c::tab::background_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.shadow = ImLerp(it_anim->second.shadow, selectable ? c::accent_color_transparency : c::tab::background_inactive, g.IO.DeltaTime * 6.f);
    ...
    ImGui::GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(0, size_arg.y / 2), bb.Max, ImGui::GetColorU32(it_anim->second.shadow), GetColorU32(it_anim->second.shadow), GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background));
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(0, size_arg.y - 1), bb.Max, GetColorU32(it_anim->second.line), 0.f);

    ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    ImGui::RenderTextClipped(bb.Min, bb.Max - ImVec2(0, it_anim->second.text_offset), label, NULL, &label_size, ImVec2(0.5, 0.5), &bb);
    ImGui::PopStyleColor();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    // Scale the checkbox size by 1.2×
    const float square_sz = GetFrameHeight() * 1.2f; // Scale factor adjusted to 1.2×
    const ImVec2 pos = window->DC.CursorPos;
    ...
    // Make the checkbox background darker
    const ImU32 checkmark_col = GetColorU32(ImVec4(0.2f, 0.2f, 0.2f, 1.0f));  // Darker check mark color
    const ImU32 active_col = GetColorU32(ImVec4(0.5f, 0.5f, 0.5f, 1.0f));      // Darker background for the active checkbox

    // If the checkbox is checked, render the check mark with a darker color
    if (*v)
    {
        const float pad = square_sz * 0.25f;
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, active_col, style.FrameRounding);
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), checkmark_col, square_sz - pad * 2.0f);
    }
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(0.2f, 0.2f, 0.2f, 1.0f), style.FrameRounding); // Darker border color

    // Render label
    if (label_size.x > 0.0f)
    {
        const ImVec2 label_pos(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + (square_sz - label_size.y) * 0.5f); // Center the label vertically
...
struct custom_tab_state
{
    ImVec4 background, unselected, text;
    float mark_animation;
    ImVec2 background_offset;
};
bool ImGui::CustomTab(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(width + padding * 2, label_size.y + padding));
    ItemSize(total_bb, style.FramePadding.y);

    // Debug: Check the bounding box dimensions
    IM_ASSERT(total_bb.Min.x >= 0 && total_bb.Min.y >= 0 && "Bounding box minimum coordinates should be non-negative");
    IM_ASSERT(total_bb.Max.x > total_bb.Min.x && total_bb.Max.y > total_bb.Min.y && "Bounding box maximum coordinates should be greater than minimum coordinates");

    if (!ItemAdd(total_bb, id))
    {
        // Debug: Log the ID and bounding box if ItemAdd fails
        IM_ASSERT(false && "ItemAdd failed");
        return false;
    }
    ...
    static std::map<ImGuiID, custom_tab_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, custom_tab_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, *v ? ImVec4(1.0f, 1.0f, 1.0f, 1.0f) : hovered ? ImVec4(0.8f, 0.8f, 0.8f, 1.0f) : ImVec4(0.5f, 0.5f, 0.5f, 1.0f), g.IO.DeltaTime * 6.f);
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? ImColor(c::accent_color) : ImColor(c::accent_color_low), g.IO.DeltaTime * 6.f);
    it_anim->second.unselected = ImLerp(it_anim->second.background, ImColor(c::accent_color_low), g.IO.DeltaTime * 6.f);

    if (*v == true)
        GetWindowDrawList()->AddRectFilled({ total_bb.Min.x,total_bb.Min.y }, { total_bb.Min.x + 3, total_bb.Max.y }, GetColorU32(it_anim->second.background), style.FrameRounding);
    else if (*v == false)
        GetWindowDrawList()->AddRectFilled({ total_bb.Min.x,total_bb.Min.y }, { total_bb.Min.x + 3, total_bb.Max.y }, GetColorU32(it_anim->second.unselected), style.FrameRounding);

    // Draw the tab background
    if(*v == true)
        GetWindowDrawList()->AddRectFilledMultiColor({ total_bb.Min.x + 3, total_bb.Min.y }, { total_bb.Max.x + 20, total_bb.Max.y }, ImColor(c::accent_color_low), ImColor(c::accent_color_transparency), ImColor(c::accent_color_transparency), ImColor(c::accent_color_low));
    if(*v == false)
        GetWindowDrawList()->AddRectFilledMultiColor({ total_bb.Min.x + 3, total_bb.Min.y }, { total_bb.Max.x + 20, total_bb.Max.y }, ImColor(20, 20, 20, 130), ImColor(7, 6, 5, 80), ImColor(7, 6, 5, 80), ImColor(20, 20, 20, 130));

    // Draw the tab label
    GetWindowDrawList()->AddText(ImVec2(total_bb.Min.x + padding, total_bb.Min.y + padding / 2), GetColorU32(it_anim->second.text), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    it_anim->second.shadow_opticaly = ImLerp(it_anim->second.shadow_opticaly, it_anim->second.opened_combo ? 1.f : 0.5f, g.IO.DeltaTime * 12.f);
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 23) + style.WindowPadding.y : 0.f, g.IO.DeltaTime * 12.f);
    ...
    GetWindowDrawList()->AddRect(bb.Min, ImVec2(value_x2 + GetFrameHeight(), bb.Max.y), GetColorU32(c::combo::background_outline), c::combo::rounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));

    RenderTextClipped(bb.Min + ImVec2(5, 3), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 3), window->DC.CursorPos + ImVec2(w, label_size.y + 18.f));
    ...
    float min_x = frame_bb.Min.x;
    float min_y = frame_bb.Min.y + 6.0f;
    float max_x = frame_bb.Max.x;
    float max_y = frame_bb.Max.y - 5.0f;

    ImVec2 outline_min(min_x, min_y);
    ImVec2 outline_max(max_x, max_y);

    GetWindowDrawList()->AddRect(outline_min, outline_max, IM_COL32(255, 105, 180, 255)); // Pink color (with full opacity)
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        // Use pink for the slider fill (track)
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 6), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y - 3), IM_COL32(255, 105, 180, 255), 100.f); // Pink fill

        // Use pink for the thumb (circle) of the slider
        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 3, grab_bb.Max.y - 5), 5.f, IM_COL32(255, 105, 180, 255), 100.f); // Pink thumb
    }

    // Set the text color to white
    PushStyleColor(ImGuiCol_Text, IM_COL32(255, 255, 255, 255)); // White color for text
    RenderTextClipped(frame_bb.Min - ImVec2(0, 22), frame_bb.Max - ImVec2(0, 17), value_buf, value_buf_end, NULL, ImVec2(1.0f, 1.0f));
    PopStyleColor();

    // Set the label color to white
    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w, frame_bb.Min.y - 22), IM_COL32(255, 255, 255, 255), label);
...
struct input_state
{
    ImVec4 hint_color, active_state;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.active_state = ImLerp(it_anim->second.active_state, ImGui::IsItemActive() ? c::accent_color_low : c::input::background_outline, g.IO.DeltaTime * 6.f);

    // Render frame
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.active_state), c::input::rounding, NULL, 1.0f);
        RenderNavHighlight(frame_bb, id);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_pos = { draw_pos.x + 7, draw_pos.y + 5 };
        ...
        if (render_cursor)
        {
            ...
            if (cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 5), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 5), GetColorU32(c::text::text_active));
                PopStyleVar();
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.move_text = ImClamp(it_anim->second.move_text + (6.f * g.IO.DeltaTime * (selected ? 10.f : -5.f)), 0.f, 10.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(text_min + ImVec2(it_anim->second.move_text, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
...
bool ImGui::Tabs(bool selected, ImTextureID image, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? ImColor(25, 26, 31, 255) : ImColor(25, 26, 31, 255), g.IO.DeltaTime * 6.f);

    it_anim->second.image = selected ? ImColor(255, 61, 129, 255).Value : (hovered ? c::text::text_hov : c::text::text);
