...
static auto UpdateCamera() -> void
{
	...
	camera::fov = read<float>(pointer->local_player + 0x4AC);
...
__forceinline fvector GetBoneLocation(uintptr_t mesh, int bone_id)
{
	uintptr_t bone_array = read<uintptr_t>(mesh + offset::bone_array);
	if (bone_array == NULL) bone_array = read<uintptr_t>(mesh + offset::bone_array + 0x10);
	ftransform bone = read<ftransform>(bone_array + (bone_id * 0x60));
	ftransform component_to_world = read<ftransform>(mesh + offset::component_to_world);
