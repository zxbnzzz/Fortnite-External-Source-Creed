...
void DrawFilledESP(int x, int y, int w, int h, float color[4], int thickness) {
	...
	float b = color[2];
	float a = color[3];
	//float a = color[3] * 0.30f;

	ImU32 converted_color = ImGui::ColorConvertFloat4ToU32(ImVec4(r, g, b, a));

	if (esp::box_type == 2) {
		ImGui::GetBackgroundDrawList()->AddRectFilled(ImVec2(x, y), ImVec2(x + w, y + h), converted_color, 10.0f, 0);
	}
	else {
		ImGui::GetBackgroundDrawList()->AddRectFilled(ImVec2(x, y), ImVec2(x + w, y + h), converted_color, 0.0f, 0);
	}
}
void DrawCorneredBox(int x, int y, int w, int h, float color[4], int thickness) {
	...
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y), ImVec2(x, y + line_h), converted_color, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y), ImVec2(x + line_w, y), converted_color, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w - line_w, y), ImVec2(x + w, y), converted_color, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w, y), ImVec2(x + w, y + line_h), converted_color, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y + h - line_h), ImVec2(x, y + h), converted_color, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x, y + h), ImVec2(x + line_w, y + h), converted_color, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w - line_w, y + h), ImVec2(x + w, y + h), converted_color, thickness);
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(x + w, y + h - line_h), ImVec2(x + w, y + h), converted_color, thickness);
}
std::string GetPlayerName(uintptr_t player_state)
{
	...
	try {
		...
		uintptr_t pNameStructure = read<uintptr_t>(player_state + 0xAD0); //pNameStructure
...
void DrawRoundedBox(int x, int y, int w, int h, float color[4], int thickness) {
	...
	ImGui::GetBackgroundDrawList()->AddRect(ImVec2(x, y), ImVec2(x + w, y + h), converted_color, 10.0f, 0, thickness);
}
	...
	public:
		...
		auto ActorLoop() -> void {
			...
			if (aimb::show_fov)
			{
				...
				ImGui::GetBackgroundDrawList()->AddCircle(ImVec2(width / 2, height / 2), aimb::fov_size, ImColor(255, 255, 255), 64, 1);
			...
			for (int i = 0; i < pointer->player_array_size; ++i) {
				...
				if (esp::rank_esp) {
					...
					uintptr_t habenero = read<uintptr_t>(player_state_enemy + 0x9e8); // https://dumpspace.spuckwaffel.com/Games/?hash=6b77eceb&type=classes&idx=AFortPlayerState&member=HabaneroComponent
				...
				if (esp::username) {
					std::string username = GetPlayerName(player_state_enemy);
					...
					/*if (is_vis) {
						DrawString(esp::text_size, head_box.x - (text_size.x / 2), head_box.y - YOffset, esp::text_color, true, esp::text_outline, username.c_str());
					}
					else {
						DrawString(esp::text_size, head_box.x - (text_size.x / 2), head_box.y - YOffset, esp::text_color, true, esp::text_outline, username.c_str());
					}*/
				...
				if (esp::distance)
				{
					...
					DrawString(esp::text_size, root_screen.x - (text_size.x / 2), root_screen.y + 25, esp::text_color, true, esp::text_outline, distance_string.c_str());
				}
				...
				if (esp::filled_esp)
				{
					if (is_vis) {
						/*if (esp::outline)
							DrawFilledESP(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, black, esp::box_thick + 3.5);*/
						DrawFilledESP(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, esp::black, esp::box_thick);
					}
					else if (!is_vis) {
						/*if (esp::outline)
							DrawFilledESP(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, black, esp::box_thick + 3.5);*/
				...
				if (esp::box && esp::box_type == 0) //Normal
				{
					...
					if (is_vis)
					{
						if (esp::outline)
						{
							DrawBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, black, esp::box_thick + 3.5);
						}
						DrawBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, esp::box_visible, esp::box_thick);
					...
					else
					{
						...
						DrawBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, esp::box_invisible, esp::box_thick);
				...
				if (esp::box && esp::box_type == 1) //Cornered
				{
					...
					if (is_vis)
					{
						if (esp::outline)
						{
							DrawCorneredBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, black, esp::box_thick + 3.5);
						}
						DrawCorneredBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, esp::box_visible, esp::box_thick);
					...
					else
					{
						...
						DrawCorneredBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, esp::box_invisible, esp::box_thick);
				...
				if (esp::box && esp::box_type == 2) //Rounded
				{
					...
					if (is_vis)
					{
						if (esp::outline)
						{
							DrawRoundedBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, black, esp::box_thick + 3.5);
						}
						DrawRoundedBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, esp::box_visible, esp::box_thick);
					...
					else
					{
						...
						DrawRoundedBox(head_box.x - (CornerWidth / 2), head_box.y, CornerWidth, CornerHeight, esp::box_invisible, esp::box_thick);
				...
				if (esp::skeleton)
				{
					//ImColor skeleton_color = esp::skeleton_visible : esp::skeleton_invisible;
					fvector2d bonePositions[16];
					const int boneIndices[] = { 110, 3, 66, 9, 38, 10, 39, 11, 40, 78, 71, 79, 72, 75, 82, 67 };
					for (int i = 0; i < 16; ++i)
					{
						bonePositions[i] = w2s(GetBoneLocation(skeletal_enemy, boneIndices[i])); 
					}
					...
					for (const auto& pair : bonePairs)
					{
						...
						if (esp::outline)
						{
							ImGui::GetBackgroundDrawList()->AddLine(start, end, ImColor(0, 0, 0, 255), esp::skeleton_thick + 2);
						}
						if (!is_vis)
						{
							ImGui::GetBackgroundDrawList()->AddLine(start, end, esp::skeleton_invisible, esp::skeleton_thick);
						}
						ImGui::GetBackgroundDrawList()->AddLine(start, end, esp::skeleton_visible, esp::skeleton_thick);
					}
